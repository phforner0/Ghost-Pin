...
class SimulationStartRequestTest {
    ...
    @Test
    fun `parseSimulationStartRequest parses valid waypoint mode request`() = runTest {
        ...
        val intent = Intent().apply {
            ...
            putExtra(SimulationService.EXTRA_REPEAT_POLICY, com.ghostpin.engine.interpolation.RepeatPolicy.LOOP_N.name)
        ...
        val result = parseSimulationStartRequest(
            intent = intent,
    ...
    @Test
    fun `parseSimulationStartRequest rejects waypoint mode with fewer than two points`() = runTest {
        val repository = SimulationRepository(
            simulationHistoryDao = FakeSimulationHistoryDao(),
            favoriteSimulationDao = FakeFavoriteSimulationDao(),
            routeDao = FakeRouteDao(),
