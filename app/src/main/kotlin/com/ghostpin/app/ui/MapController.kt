...
class MapController(
    ...
    init {
        // Load a free vector style (e.g., standard demo style)
        map.setStyle(Style.getPredefinedStyle("https://demotiles.maplibre.org/style.json")) { loadedStyle ->
    ...
    fun updatePosition(location: MockLocation) {
        ...
        // meters-per-pixel at current latitude and zoom, but a static multiplier is fine for visual demo.
        val zoomBase = map.cameraPosition.zoom
        val metersPerPixel = 156543.03392 * Math.cos(location.lat * Math.PI / 180.0) / Math.pow(2.0, zoomBase)
    ...
    fun clearRoute() {
        style?.getSourceAs<GeoJsonSource>(SOURCE_ROUTE)?.setGeoJson(FeatureCollection.fromFeatures(emptyList()))
    }
    
    fun clearPosition() {
        style?.getSourceAs<GeoJsonSource>(SOURCE_POSITION)?.setGeoJson(FeatureCollection.fromFeatures(emptyList()))
    }
