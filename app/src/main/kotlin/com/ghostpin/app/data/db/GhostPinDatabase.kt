    ...
    companion object {
        ...
        val MIGRATION_4_5 =
            object : Migration(4, 5) {
                override fun migrate(db: SupportSQLiteDatabase) {
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `startLat` REAL NOT NULL DEFAULT -23.5505"
                    )
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `startLng` REAL NOT NULL DEFAULT -46.6333"
                    )
                    ...
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `appMode` TEXT NOT NULL DEFAULT 'CLASSIC'"
                    )
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `waypointsJson` TEXT NOT NULL DEFAULT '[]'"
                    )
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `waypointPauseSec` REAL NOT NULL DEFAULT 0.0"
                    )
                    db.execSQL(
                        "ALTER TABLE `favorite_simulations` ADD COLUMN `repeatPolicy` TEXT NOT NULL DEFAULT 'NONE'"
                    )
                    ...
                    db.execSQL(
                        "ALTER TABLE `simulation_history` ADD COLUMN `waypointPauseSec` REAL NOT NULL DEFAULT 0.0"
                    )
                    ...
                    db.execSQL(
                        "ALTER TABLE `simulation_history` ADD COLUMN `repeatPolicy` TEXT NOT NULL DEFAULT 'NONE'"
                    )
        ...
        val MIGRATION_5_6 =
            object : Migration(5, 6) {
                override fun migrate(db: SupportSQLiteDatabase) {
                    db.execSQL("ALTER TABLE `favorite_simulations` ADD COLUMN `profileName` TEXT NOT NULL DEFAULT ''")
                    db.execSQL(
                        "UPDATE `favorite_simulations` SET `profileName` = `profileIdOrName` WHERE `profileName` = ''"
                    )

                    db.execSQL("ALTER TABLE `simulation_history` ADD COLUMN `profileName` TEXT NOT NULL DEFAULT ''")
                    db.execSQL(
                        "UPDATE `simulation_history` SET `profileName` = `profileIdOrName` WHERE `profileName` = ''"
                    )

                    db.execSQL("ALTER TABLE `schedules` ADD COLUMN `profileLookupKey` TEXT NOT NULL DEFAULT ''")
                    db.execSQL(
                        "UPDATE `schedules` SET `profileLookupKey` = `profileName` WHERE `profileLookupKey` = ''"
                    )
                }
            }
    }
