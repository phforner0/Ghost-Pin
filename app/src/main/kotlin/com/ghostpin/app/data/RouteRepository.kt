...
class RouteRepository
    ...
    constructor(
        ...
        fun observeAll(): Flow<List<Route>> = dao.observeAll().map { entities -> entities.mapNotNull { it.toDomain() } }
        ...
        suspend fun update(
            ...
            val entity =
                route.toEntity(sourceFormat).copy(
        ...
        private fun RouteEntity.toDomain(): Route? =
            runCatching {
                ...
                val waypoints =
                    (0 until waypointArr.length()).map { i ->
                ...
                val segments =
                    (0 until segmentArr.length()).map { i ->
                        ...
                        val pauseDuration =
                            if (obj.isNull(
                                ...
                                obj.getDouble("pauseDurationSec")
                            }
                        val loop = obj.optBoolean("loop", false)
                        val overrides =
                            if (speedOverride != null || pauseDuration != null || loop) {
